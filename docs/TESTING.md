...
- **Uma rodada de correção de achado cujo diff não toca `src/`/`frontend/`** (só acrescenta casos
  ...
  (`.claude/skills/sdd-implement/SKILL.md`, seção "Retomando para corrigir achados de revisão",
  passo 3a). Correção que toca comportamento de produção não entra nessa exceção: ali a suíte
  completa local continua obrigatória, e o `Commit` do artefato de cobertura precisa ser regravado.
