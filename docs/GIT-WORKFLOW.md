...
2. **Uma branch por feature/spec**, criada a partir de `main` atualizada, com o nome
   ...
   o vínculo spec ↔ código ↔ branch ↔ PR rastreável só pelo nome.
...
4. **QA e SRE revisam contra o PR**, não contra código local solto: o `qa-report.md` e o
   `sre-review.md` da feature referenciam o número/link do PR.
5. **Merge só depois de QA e SRE aprovados** e CI verde (lint + testes + gate de cobertura 80%).
   Preferência por *squash merge* — um commit por feature em `main`, histórico linear e legível.
...
| `/sdd-implement`       | Cria `feature/<NNNN-slug>` a partir de `main`; abre PR draft no primeiro commit; commita incrementalmente (um commit por ciclo TDD ou por incremento coerente). |
