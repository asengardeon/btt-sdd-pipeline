...
4. **PR aberto cedo**, em modo *draft*, no primeiro commit — não só no final (mantém o CI rodando
   ...
   de `/sdd-sre` quando a fatia é aprovada (`.claude/skills/sdd-sre/SKILL.md`, passo 5d) — nunca
   fica como transição manual pendente para o usuário perceber sozinho. "Commita por incremento"
   (`backend-developer`/
...
| `/sdd-sre`              | CI/CD/infra impactados por esta fatia; aprovado = PR pronto para merge (tira o PR do modo draft, `gh pr ready`); commita e envia (push) `sre-review.md` (e qualquer ajuste de infra desta rodada) na mesma branch. |
