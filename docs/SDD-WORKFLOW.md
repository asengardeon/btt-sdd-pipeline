...
## Utilitário: `/sdd-project-conventions`

Sem posição fixa numa etapa, no mesmo espírito de `/sdd-docs`. Aciona `codebase-archaeologist` numa
responsabilidade separada da Etapa 0: em vez de documentar o sistema (`docs/BASELINE.md`),
...
automaticamente ao final de `/create-project`, ou de carona numa rodada de `/sdd-baseline` que já
está inspecionando o projeto.
...
reabre o pipeline completo do zero. `/sdd-hotfix` (`.claude/skills/sdd-hotfix/SKILL.md`) formaliza
