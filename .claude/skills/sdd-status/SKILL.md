...
6. **Docs desatualizados em relação à versão instalada do plugin** (útil em projetos criados há um
   ...
   scaffold de `/create-project` da instalação atual e só sinaliza divergência (arquivo + versão do
   scaffold usada na comparação) — nunca aplica merge automático; cabe ao usuário revisar e decidir
   se quer atualizar manualmente. Não é o comportamento padrão (custo extra de I/O) — só rode com
   essa flag quando o usuário perguntar especificamente sobre isso, ou quando uma sessão notar algo
   que parece um gap de instrução ausente no projeto.
