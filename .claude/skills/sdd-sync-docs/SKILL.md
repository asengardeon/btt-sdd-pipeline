...
7. Commite as mudanças (`docs/*.md`/`CLAUDE.md` tocados, mais `docs/.sdd-plugin-version`) numa
   ...
   de `/repo-issues`, que é exclusivo do repositório de origem do plugin) — o merge fica a critério
   do usuário, como em qualquer PR normal deste pipeline.
...
## Quando outro comando `/sdd-*` te aciona automaticamente
...
comando original — não é preciso o usuário invocar `/sdd-sync-docs` de novo separadamente.
