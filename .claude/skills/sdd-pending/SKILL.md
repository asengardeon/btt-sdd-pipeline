...
1. **Prefira o script auxiliar em vez de ler cada artefato inteiro** (economiza tokens): rode
   `scripts/sdd-pending.sh [slug]` (bash) ou `scripts/sdd-pending.ps1 [-Slug <slug>]`
   (PowerShell), a partir da raiz do projeto. Ele já varre `prd.md`, `trd.md`, `code-review.md`,
...
3. **Só leia os artefatos manualmente** (fallback abaixo) se o script não existir no projeto
   atual ou falhar.
