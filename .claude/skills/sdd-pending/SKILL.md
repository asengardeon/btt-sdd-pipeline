...
Não aciona nenhum agente — é um utilitário de leitura, como `/sdd-status`, focado em duas coisas:
os itens marcados "VALIDAR DEPOIS" e as tarefas que a tabela de decomposição de cada TRD (seção
"Decomposição de tarefas e dependências") ainda não marca como implementadas. Diferente de
`/sdd-gap-report` (que compara os casos de uso do TRD, seção 6, com o código real) — aqui a fonte
...
4. Se o usuário pedir para resolver um item específico de VALIDAR DEPOIS (responder a pergunta
   agora), colete a resposta e acione `/sdd-amend` para registrar a resolução no artefato
   ...
   real (`docs/POST-MERGE-VALIDATION.md`), a mesma lógica se aplica: feche via `/sdd-amend`, nunca
...
5. Se o usuário pedir para avançar numa tarefa específica da tabela de "Tarefas ainda não
   implementadas", **não** é caso de `/sdd-amend` (não é uma decisão já aprovada sendo emendada) —
   aponte para a skill certa conforme o Status da linha: `pendente`/`em andamento`/`bloqueado` sem
   código ainda apontam para `/sdd-implement` na fatia correspondente; se o TRD nem existe ainda
   para aquela feature (linha "Specs com PRD aprovado mas sem TRD"), aponte para `/sdd-trd`
   primeiro.
