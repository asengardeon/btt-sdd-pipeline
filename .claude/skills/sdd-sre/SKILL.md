...
2b. **Se a branch/PR original da fatia já foi mergeado e apagado** (comum em revisão retroativa
   pedida depois do fato — ex.: um `/sdd-hotfix` que pulou SRE no momento do merge, e o usuário
...
5b. **Spec finalizada → aciona `tech-writer` automaticamente.** Se a fatia aprovada (ou aprovada
   ...
   `subagent_type: "tech-writer"`, ou `/sdd-docs`) para atualizar a documentação do repositório
   ...
   usuário que, depois do merge desta última fatia, `/sdd-implement` conduz o teste geral
