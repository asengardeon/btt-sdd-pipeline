...
5. **Só depois de concluídos todos os incrementos da sua trilha**, rode a suíte completa com
   ...
   preciso rodar a suíte de novo — só atualize o campo `Commit` de `coverage/<fatia>-backend.md`
   ...
   de backend desta fatia no TRD para `implementado`, refletindo a mesma transição na Issue GitHub
   associada, se houver.
...
5b. **Antes de reportar a trilha como concluída, confirme contra `origin/<branch>` — não só que
   ...
   `coverage/<fatia>-backend.md`, a atualização da coluna Status no TRD, etc.) está de fato
