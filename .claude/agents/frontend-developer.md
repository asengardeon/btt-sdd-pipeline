...
3. Apresente esse plano ao usuário via `AskUserQuestion` e **só prossiga para a Fase 2 com
   ...
   - **Se `AskUserQuestion` não estiver disponível nesta invocação** (trilha única rodando como
     ...
     (`/sdd-implement`) apresentar esse plano ao usuário via a própria `AskUserQuestion` e retomar
     você (`SendMessage`) com a aprovação antes que a Fase 2 comece. Já aconteceu de verdade de uma
     invocação isolada sem esse gate seguir em frente com a leitura mais conservadora do TRD e só
     reportar isso a posteriori no resumo final — funcionou por sorte (TRD inequívoco), mas não é
     o comportamento esperado.

## Fase 2 — Execução

1. Identifique a fatia desta rodada e o nome de branch (TRD, seção "Controle de versão (GitHub
   Flow, por fatia)", ou instrução do orquestrador de `/sdd-implement`). Se não é a primeira
   ...
   automaticamente as issues correspondentes no GitHub. **Repita a palavra-chave por issue** —
   `Closes #117, Closes #118, Closes #119` — nunca a lista com vírgulas simples (`Closes #117,
...
5c. **Se esta trilha tocou mais de um ponto de entrada estruturalmente equivalente** (mesma
   ...
   pontos de upload usando o mesmo hook compartilhado saiu com teste de integração dedicado em só
   5 deles — os outros 2 só ganharam teste depois que `code-reviewer` identificou a assimetria
   numa rodada extra evitável.
...
terminou. Se não há trilha de backend pendente, a próxima etapa é `/sdd-code-review` com o
