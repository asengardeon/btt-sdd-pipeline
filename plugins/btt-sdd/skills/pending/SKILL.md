...
1. **Prefira o script auxiliar em vez de ler cada artefato inteiro** (economiza tokens): rode
   ...
   com status "pendente" e devolve uma tabela: feature | artefato | ID | pergunta | contexto.
...
4. Se o usuário pedir para resolver um item específico (responder a pergunta agora), colete a
   ...
   edite o artefato diretamente por fora desse fluxo, para o log de revisões ficar consistente. Se
   o item já foi confirmado por uma validação manual pós-merge contra produção real
   (`docs/POST-MERGE-VALIDATION.md`), a mesma lógica se aplica: feche via `/btt-sdd:amend`, nunca
