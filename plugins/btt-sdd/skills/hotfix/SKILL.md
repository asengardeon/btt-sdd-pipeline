...
5. **Decida as revisões desta rodada** — critério objetivo, não uma decisão manual reavaliada a
   ...
   - **QA: se há um critério de aceite concreto a validar.** Se a correção reafirma um
     ...
     `/btt-sdd:status`/`/btt-sdd:pending` (`skills/status/scripts/sdd-status.sh`/`.ps1`)
     reconhecerem isso como um estado terminal válido em vez de "veredito não identificado",
   ...
   - **UX/Usabilidade: sempre que o diff alterar uma tela/fluxo com superfície de UI perceptível
     ...
     vazio/erro, conteúdo de mídia) — roda `/btt-sdd:ux-review` contra o PR desta rodada. Uma
     correção puramente de backend/infra sem nenhuma tela afetada pula esta etapa, registrando a
...
6b. **Antes de informar que o merge fica a critério do usuário, confirme explicitamente que toda
   ...
   "Histórico de aprovações por fatia", linha `hotfix-<data>`): se qualquer revisão marcada como
   aplicável ainda
